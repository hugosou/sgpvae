...
kernel = sgpvae.kernels.RBFKernel(
    lengthscale=init_lengthscale, scale=init_scale)
w_transform = torch.sigmoid if transform else None

# Likelihood function.
if likelihood == 'gprn':
    print('Using GPRN likelihood function.')
    likelihood = sgpvae.likelihoods.GPRNHomoGaussian(
        f_dim=f_dim, out_dim=y.shape[1], sigma=sigma,
        w_transform=w_transform)
    latent_dim = f_dim + f_dim * y.shape[1]

elif likelihood == 'gprn-nn':
    print('Using GPRN-NN likelihood function.')
    likelihood = sgpvae.likelihoods.GPRNNNHomoGaussian(
        f_dim=f_dim, w_dim=w_dim, out_dim=y.shape[1],
        hidden_dims=decoder_dims, sigma=sigma,
        w_transform=w_transform)
    latent_dim = f_dim + f_dim * w_dim

elif likelihood == 'nn-gprn':
    print('Using NN-GPRN likelihood function.')
    likelihood = sgpvae.likelihoods.NNGPRNHomoGaussian(
        f_dim=f_dim, w_dim=w_dim, out_dim=y.shape[1],
        hidden_dims=decoder_dims, sigma=sigma)
    latent_dim = f_dim + w_dim * y.shape[1]

elif likelihood == 'linear':
    print('Using linear likelihood function.')
    likelihood = sgpvae.likelihoods.AffineHomoGaussian(
        in_dim=latent_dim, out_dim=y.shape[1], sigma=sigma)

else:
    print('Using NN likelihood function.')
    likelihood = sgpvae.likelihoods.NNHomoGaussian(
        in_dim=latent_dim, out_dim=y.shape[1],
        hidden_dims=decoder_dims, sigma=sigma)
    latent_dim = latent_dim

# Approximate likelihood function.
if pinference_net == 'factornet':
    variational_dist = sgpvae.likelihoods.FactorNet(
        in_dim=y.shape[1], out_dim=latent_dim,
        h_dims=h_dims, min_sigma=min_sigma,
        init_sigma=initial_sigma)

elif pinference_net == 'indexnet':
    variational_dist = sgpvae.likelihoods.IndexNet(
        in_dim=y.shape[1], out_dim=latent_dim,
        inter_dim=inter_dim, h_dims=h_dims,
        rho_dims=rho_dims, min_sigma=min_sigma,
        init_sigma=initial_sigma)

elif pinference_net == 'pointnet':
    variational_dist = sgpvae.likelihoods.PointNet(
        out_dim=latent_dim, inter_dim=inter_dim,
        h_dims=h_dims, rho_dims=rho_dims,
        min_sigma=min_sigma, initial_sigma=initial_sigma)

elif pinference_net == 'zeroimputation':
    variational_dist = sgpvae.likelihoods.NNHeteroGaussian(
        in_dim=y.shape[1], out_dim=latent_dim,
        hidden_dims=h_dims, min_sigma=min_sigma,
        init_sigma=initial_sigma)
...
if model == 'gpvae':
    ...
    z_init = torch.linspace(
        0, x[-1].item(), steps=num_inducing, device=device).unsqueeze(1)
    ...
    model = sgpvae.models.SGPVAE(
        ...
        add_jitter=add_jitter, fixed_inducing=fixed_inducing)
    ...
    model = sgpvae.models.VAE(
        likelihood, variational_dist, latent_dim)

else:
    raise ValueError('{} is not a model.'.format(model))
